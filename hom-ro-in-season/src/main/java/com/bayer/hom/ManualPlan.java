		...
		for (int i = 14; i <= nrows; i++) {
			...
			if (!checkIfRowIsEmpty(row)) {
				...
				if (hFields.containsKey(tracking_number)) {
					slf4jLogger.warn("[Manual Plan Excel - Reading Excel file] duplicated entry for field {}", tracking_number);
				} else {
					hFields.put(tracking_number, f);
				}
				slf4jLogger.debug("[Manual Plan Excel - Reading Excel file] Field {}", f);
			}
		}
		wb.close();
	}
